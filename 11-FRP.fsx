...
type EventStore() =
    ...
    member this.Get<'a>(eventId) =
        ...
        | true, eventList ->
            eventList
            |> Seq.cast<'a>
            |> Seq.toList // convert to typed list
...
module EventProcessors =
    ...
    let physicalTurtleProcessor (eventStream: IObservable<Guid * obj>) =
        ...
        let subscriberFn (ev: MovedEvent) =
            ...
            printfn
                "[turtle  ]: Moved from (%0.2f,%0.2f) to (%0.2f,%0.2f) with %s"
                ev.startPos.x
                ev.startPos.y
                ev.endPos.x
                ev.endPos.y
    ...
    let graphicsProcessor (eventStream: IObservable<Guid * obj>) =
        ...
        let subscriberFn (ev: MovedEvent) =
            ...
            | Some color ->
                printfn
                    "[graphics]: Draw line from (%0.2f,%0.2f) to (%0.2f,%0.2f) with color %A"
                    ev.startPos.x
                    ev.startPos.y
                    ev.endPos.x
                    ev.endPos.y
