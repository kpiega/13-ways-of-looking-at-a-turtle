...
module TurtleProgram_v1 =
    ...
    type TurtleProgram =
        ...
        | PenUp (* none *) of (unit -> TurtleProgram)
        | PenDown (* none *) of (unit -> TurtleProgram)
                    ...
                    fun () ->
                        Move(
                            ...
                            fun response ->
                                Turn(
                                    120.0<Degrees>,
                                    fun () -> Move(100.0, (fun response -> Turn(120.0<Degrees>, (fun () -> Stop))))
                                )
                        )
                )
        )
...
module TurtleProgram_v1_Workflow =
    ...
    let rec bindT f inst =
        ...
        | PenUp (next) -> PenUp(next >> bindT f)
        | PenDown (next) -> PenDown(next >> bindT f)
        | SetColor (color, next) -> SetColor(color, next >> bindT f)
...
module TurtleProgram_v1_WorkflowExample =
    ...
    let handleMoveResponse log moveResponse =
        turtleProgram {
